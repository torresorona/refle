"""Tier 1 open-core admin: control scoping, audit log, org switching."""
...
async def test_switch_org(client):
    ...
    # Add the user to a second org as admin.
    ...
    assert switched.json()["organization_id"] == str(org_b_id)
    assert switched.json()["role"] == "admin"

    # Switching to an org the user doesn't belong to is forbidden.
